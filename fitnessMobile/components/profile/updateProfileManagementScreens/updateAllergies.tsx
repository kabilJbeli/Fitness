...
const UpdateAllergies = (props: any) => {
  ...
  useEffect(() => {
    _retrieveData("memberInfo").then((info: any) => {
      ...
      if (parsedInfo !== undefined) {
        ...
        setAllergies(parsedInfo.allergiedescription);
      }
    });
  }, [props]);
  ...
  const updateClient = (pictures: any[]) => {
    ...
    payload.allergie = allergies.trim().length > 0 ? true : false;
    payload.allergiedescription = allergies;
    axios({
      method: "PUT",
  ...
  return (
    <View style={{
      ...
      <View style={{
        ...
        <View>
          <Text style={{ textAlign: "center", fontWeight: "bold", fontSize: 32, color: "#000", marginBottom: 5 }}>Présence
            d’allergie ou des problèmes de santé?</Text>
        ...
        <ScrollView style={{ height: Dimensions.get("window").height * 0.6}} contentContainerStyle={{ flexGrow: 1,justifyContent:"flex-end" }}>
          <TextInput
            style={{
              flex: 1,
